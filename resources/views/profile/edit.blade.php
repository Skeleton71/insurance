    ...
    <div class="row">
        <div class="col-md-8 offset-md-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <p class="text-muted">{{ __('messages.delete_warning') }}</p>

                    <form method="POST" action="{{ route('profile.destroy') }}" style="display: inline-block;">
                        @csrf
                        @method('DELETE')
                        <input type="password" name="password" class="form-control mb-3" placeholder="{{ __('messages.enter_password') }}" required>
                        <button type="submit" class="btn btn-danger" onclick="return confirm('{{ __('messages.are_you_sure') }}')">{{ __('messages.delete_account') }}</button>
                    </form>
                </div>
            </div>
        </div>
